...
with open(r'E:\cloud9_hackathon\extract_log_vod8_v4.txt', 'r', encoding='utf-16') as f:
    lines = f.readlines()

LEFT_TEAM = "NV"
RIGHT_TEAM = "C9"
LEFT_PLAYERS = {"keznit", "rossy", "p0ppin", "eggsterr", "inspire"}
RIGHT_PLAYERS = {"penny", "demon1", "zellsis", "v1c", "xeppaa"}
...
score_after = {}
for t, ls, rs in round_transitions:
    # Find which round this transition ends
    for rnd_num in sorted(rounds.keys()):
        rnd_kills = rounds[rnd_num]
        if rnd_kills:
            last_t = max(k[0] for k in rnd_kills)
            if abs(t - last_t) < 10:  # within 10s
                score_after[rnd_num] = (ls, rs)
                break

# Alternative: assign scores sequentially
...
# Build output
output_lines = []
output_lines.append(f"VOD 8 — Round-by-Round Kill Breakdown (2s Replay Filter)")
output_lines.append(f"{LEFT_TEAM} (left): keznit, Rossy, P0PPIN, Eggsterr, Inspire")
output_lines.append(f"{RIGHT_TEAM} (right): penny, Demon1, Zellsis, v1c, Xeppaa")
...
result = "\n".join(output_lines)
with open(r'E:\cloud9_hackathon\vod8_killfeed_breakdown.txt', 'w', encoding='utf-8') as f:
    f.write(result)
print(result)
